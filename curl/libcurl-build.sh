...
buildMac()
{
	...
	pushd . > /dev/null
	cd "${CURL_VERSION}"
	./configure -prefix="/tmp/${CURL_VERSION}-${ARCH}" $CONF_FLAGS --with-secure-transport --with-ssl=${OPENSSL}/Mac ${NGHTTP2CFG} ${LIBSSH2CFG} --host=${HOST} &> "/tmp/${CURL_VERSION}-${ARCH}.log"
...
buildCatalyst()
{
	...
	if [[ "${ARCH}" == "arm64" ]]; then
		./configure -prefix="/tmp/${CURL_VERSION}-catalyst-${ARCH}-${BITCODE}" $CONF_FLAGS --with-secure-transport --with-ssl=${OPENSSL}/catalyst ${NGHTTP2CFG} ${LIBSSH2CFG} --host="arm-apple-darwin" &> "/tmp/${CURL_VERSION}-catalyst-${ARCH}-${BITCODE}.log"
	else
		./configure -prefix="/tmp/${CURL_VERSION}-catalyst-${ARCH}-${BITCODE}" $CONF_FLAGS --with-secure-transport --with-ssl=${OPENSSL}/catalyst ${NGHTTP2CFG} ${LIBSSH2CFG} --host="${ARCH}-apple-darwin" &> "/tmp/${CURL_VERSION}-catalyst-${ARCH}-${BITCODE}.log"
	fi
...
buildIOS()
{
	...
	if [[ "${ARCH}" == *"arm64"* || "${ARCH}" == "arm64e" ]]; then
		./configure -prefix="/tmp/${CURL_VERSION}-iOS-${ARCH}-${BITCODE}" $CONF_FLAGS --with-secure-transport --with-ssl=${OPENSSL}/${PLATFORMDIR} ${NGHTTP2CFG} ${LIBSSH2CFG} --host="arm-apple-darwin" &> "/tmp/${CURL_VERSION}-iOS-${ARCH}-${BITCODE}.log"
	else
		./configure -prefix="/tmp/${CURL_VERSION}-iOS-${ARCH}-${BITCODE}" $CONF_FLAGS --with-secure-transport --with-ssl=${OPENSSL}/${PLATFORMDIR} ${NGHTTP2CFG} ${LIBSSH2CFG} --host="${ARCH}-apple-darwin" &> "/tmp/${CURL_VERSION}-iOS-${ARCH}-${BITCODE}.log"
	fi
...
buildIOSsim()
{
	...
	if [[ "${ARCH}" == *"arm64"* || "${ARCH}" == "arm64e" ]]; then
		./configure -prefix="/tmp/${CURL_VERSION}-iOS-simulator-${ARCH}-${BITCODE}" $CONF_FLAGS --with-secure-transport --with-ssl=${OPENSSL}/${PLATFORMDIR} ${NGHTTP2CFG} ${LIBSSH2CFG} --host="arm-apple-darwin" &> "/tmp/${CURL_VERSION}-iOS-simulator-${ARCH}-${BITCODE}.log"
	else
		./configure -prefix="/tmp/${CURL_VERSION}-iOS-simulator-${ARCH}-${BITCODE}" $CONF_FLAGS --with-secure-transport --with-ssl=${OPENSSL}/${PLATFORMDIR} ${NGHTTP2CFG} ${LIBSSH2CFG} --host="${ARCH}-apple-darwin" &> "/tmp/${CURL_VERSION}-iOS-simulator-${ARCH}-${BITCODE}.log"
	fi
...
#	export PKG_CONFIG_PATH
	...
	./configure -prefix="/tmp/${CURL_VERSION}-tvOS-${ARCH}" --host="arm-apple-darwin" $CONF_FLAGS --with-secure-transport --disable-shared -with-random=/dev/urandom --disable-ntlm-wb --with-ssl="${OPENSSL}/tvOS" ${NGHTTP2CFG} ${LIBSSH2CFG} &> "/tmp/${CURL_VERSION}-tvOS-${ARCH}.log"
...
buildTVOSsim()
{
	...
	if [[ "${ARCH}" == "arm64" ]]; then
		./configure --prefix="/tmp/${CURL_VERSION}-tvOS-simulator-${ARCH}" --host="arm-apple-darwin" $CONF_FLAGS --with-secure-transport --disable-shared -with-random=/dev/urandom --disable-ntlm-wb --with-ssl="${OPENSSL}/${PLATFORMDIR}" ${NGHTTP2CFG} ${LIBSSH2CFG} &> "/tmp/${CURL_VERSION}-tvOS-simulator-${ARCH}.log"
	else
		./configure --prefix="/tmp/${CURL_VERSION}-tvOS-simulator-${ARCH}" --host="${ARCH}-apple-darwin" $CONF_FLAGS --with-secure-transport --disable-shared  -with-random=/dev/urandom --disable-ntlm-wb --with-ssl="${OPENSSL}/${PLATFORMDIR}" ${NGHTTP2CFG} ${LIBSSH2CFG} &> "/tmp/${CURL_VERSION}-tvOS-simulator-${ARCH}.log"
	fi
